...
type CSharpLspServer(lspClient: CSharpLspClient, settings: ServerSettings) =
    ...
    let requestHandler requestName (handler: ActivateServerRequest -> 'a -> Async<LspResult<'b>>) requestParams =
        ...
        let activateRequest mode targetUri = async {
            ...
            return
                ServerRequestContext(
                    ...
                    fun e -> emittedEvents.Add(e :?> ServerEvent)
                )
        }

        let terminateRequest cause (_: obj) =
            stateActor.Post(TerminateRequest(requestId, cause, emittedEvents |> List.ofSeq))

        let requestProcessing = async {
            let handlerAsync = handler activateRequest requestParams
            let! outcome = handlerAsync |> Async.Catch

            do terminateRequest RanToCompletion ()
        ...
        Async.TryCancelled(requestProcessing, terminateRequest Cancelled)
