...
let getRazorDocumentForUri
    (solution: Solution)
    (uri: string)
    : Async<(Project * Compilation * string * SyntaxTree) option> =
    async {
        let cshtmlPath = Uri.toPath uri
        ...
        return
            cshtmlTree
            |> Option.map (fun cst -> (projectForPath.Value, compilation, cshtmlPath, cst))
    }
