...
module CodeAction =
            ...
            for refactoringProvider in refactoringProviderInstances do
                try
                    do!
                        ...
                        |> Async.AwaitTask
                with ex ->
                    logger.LogError(ex, "cannot compute refactorings for {provider}", refactoringProvider)
    ...
    let lspDocChangesFromSolutionDiff
        ...
        async {
            let originalSolution = workspaceFolderLoadedSolutionOrExn wf
            ...
            for docId in addedDocs do
                let newDoc =
                    updatedSolution.GetDocument(docId)
                    |> nonNull "updatedSolution.GetDocument(docId)"
    ...
    let resolve (acquireContext: ActivateServerRequest) (p: CodeAction) : AsyncLspResult<CodeAction> = async {
        ...
        let! context = acquireContext ReadOnlySequential (Some resolutionData.Value.TextDocumentUri)
