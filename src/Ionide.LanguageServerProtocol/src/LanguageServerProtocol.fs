...
module Server =
  ...
  let internal jeToJToken (je: System.Text.Json.JsonElement) : JToken =
  ...
  type JsonElementConverter() =
    ...
    override _.CanConvert(t) = t = typeof<System.Text.Json.JsonElement>

    override _.WriteJson(writer: Newtonsoft.Json.JsonWriter, value: obj, _serializer: JsonSerializer) =
      jeToJToken(value :?> System.Text.Json.JsonElement).WriteTo(writer)

    override _.ReadJson(reader: Newtonsoft.Json.JsonReader, _t: System.Type, _existing: obj, _serializer: JsonSerializer) =
      jtokenToJe (JToken.ReadFrom(reader)) :> obj
...
module Client =
  ...
  type NotificationHandler = { Run: System.Text.Json.JsonElement -> Async<System.Text.Json.JsonElement option> }

  let notificationHandling<'p, 'r> (handler: 'p -> Async<'r option>) : NotificationHandler =
    let run (je: System.Text.Json.JsonElement) =
      async {
        try
          let p = je |> jeToJToken |> (fun t -> t.ToObject<'p>(jsonSerializer))
          let! res = handler p

          return
            res
            |> Option.map (fun n -> JToken.FromObject(n, jsonSerializer) |> jtokenToJe)
        with _ ->
          return None
      }
  ...
  type Client(exec: string, args: string, notificationHandlings: Map<string, NotificationHandler>) =
    ...
    member __.SendNotification (rpcMethod: string) (requestObj: obj) =
      let serializedResponse = JToken.FromObject(requestObj, jsonSerializer) |> jtokenToJe
