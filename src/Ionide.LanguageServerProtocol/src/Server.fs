...
type LspServer() =
  ...
  abstract member TextDocumentDocumentHighlight: DocumentHighlightParams -> AsyncLspResult<DocumentHighlight[] option>
  ...
  /// returned.
  abstract member TextDocumentMoniker: MonikerParams -> AsyncLspResult<Moniker[] option>
  ...
  abstract member TextDocumentLinkedEditingRange: LinkedEditingRangeParams -> AsyncLspResult<LinkedEditingRanges option>
  ...
  default __.WindowWorkDoneProgressCancel(_) = ignoreNotification
  ...
  interface ILspServer with
    ...
    member this.TextDocumentDeclaration(p: DeclarationParams) = this.TextDocumentDeclaration(p)
    member this.TextDocumentDefinition(p: DefinitionParams) = this.TextDocumentDefinition(p)
    member this.TextDocumentReferences(p: ReferenceParams) = this.TextDocumentReferences(p)
    member this.TextDocumentDocumentHighlight(p: DocumentHighlightParams) = this.TextDocumentDocumentHighlight(p)
    member this.TextDocumentDocumentLink(p: DocumentLinkParams) = this.TextDocumentDocumentLink(p)
    member this.TextDocumentTypeDefinition(p: TypeDefinitionParams) = this.TextDocumentTypeDefinition(p)
    member this.TextDocumentImplementation(p: ImplementationParams) = this.TextDocumentImplementation(p)
    ...
    member this.TextDocumentMoniker(p: MonikerParams) = this.TextDocumentMoniker(p)
    member this.TextDocumentLinkedEditingRange(p: LinkedEditingRangeParams) = this.TextDocumentLinkedEditingRange(p)
    ...
    member this.WindowWorkDoneProgressCancel(p: WorkDoneProgressCancelParams) = this.WindowWorkDoneProgressCancel(p)
