...
type DefinitionTests() =
    static let mutable client: ClientController =
        setupServerClient defaultClientProfile "TestData/testDefinitionWorks"

    [<OneTimeSetUp>]
    member _.Setup() = client.StartAndWaitForSolutionLoad()

    [<Test>]
    member _.``textDocument/definition works in .cs file``() =
    ...
    [<Test>]
    member _.``textDocument/definition works in aspnet project``() =
