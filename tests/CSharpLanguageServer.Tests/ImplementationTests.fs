...
[<TestFixture>]
type ImplementationTests() =
    inherit SharedReadOnlyFixture("genericProject", "Project/ClassAndInterfaceHierarchy.cs")

    [<Test>]
    member this.testTextDocumentImplementationWorks() =
        let client = this.Client
        let hierarchyFile = this.Doc
    ...
    [<Test>]
    member this.testImplementationOnConcreteClassWorks() =
    ...
    [<Test>]
    member this.testImplementationOnAbstractClassWorks() =
    ...
    [<Test>]
    member this.testImplementationOnInterfaceTypeWorks() =
